...
namespace LLProc {
    ...
    TlmChanImplTester::TlmChanImplTester(LLProc::LLTlmChanImpl& inst) :
        LLTlmChanGTestBase("testerbase",100),
            m_impl(inst),
            m_numBuffs(0),
            m_bufferRecv(false) {
    }
...
//        for (NATIVE_UINT_TYPE n=0; n <= TLMCHAN_HASH_BUCKETS; n++) {
    ...
    void TlmChanImplTester::dumpTlmEntry(LLTlmChanImpl::TlmEntry* entry) {
        printf(
                "Entry "
                " Ptr: %p"
                " id: 0x%08X"
...
//        printf("**Buffer 0\n");
        for (NATIVE_INT_TYPE slot = 0; slot < TLMCHAN_NUM_TLM_HASH_SLOTS; slot++) {
            ...
            if (m_impl.m_tlmEntries[0].slots[slot]) {
                LLTlmChanImpl::TlmEntry* entry = m_impl.m_tlmEntries[0].slots[slot];
...
//                LLTlmChanImpl::TlmEntry* entry = m_impl.m_tlmEntries[1].slots[slot];
//                for (NATIVE_INT_TYPE bucket = 0; bucket < TLMCHAN_HASH_BUCKETS; bucket++) {
//                    dumpTlmEntry(entry);
//                    if (entry->next == 0) {
//                        break;
