// ======================================================================
// \title  BlspSerialDriver.hpp
// \author tcanham
// \brief  cpp file for BlspSerialDriver test harness implementation class
//
// \copyright
...
namespace SnapdragonFlight {
  ...
  Tester ::
    Tester(NATIVE_INT_TYPE device, NATIVE_INT_TYPE numReadBuffers, NATIVE_INT_TYPE bufferSize,
           BlspSerialDriverComponentImpl::UartFlowControl flow) :
#if FW_OBJECT_NAMES == 1
    BlspSerialDriverTesterBase("Tester", MAX_HISTORY_SIZE),
      component("BlspSerialDriver")
#else
  BlspSerialDriverTesterBase(MAX_HISTORY_SIZE),
      component()
#endif
     ,m_numBuffers(numReadBuffers),m_bufferSize(bufferSize)
  {
    ...
    // configure port
    this->component.open(device,BlspSerialDriverComponentImpl::BAUD_921K,flow,true);
    ...
    ~Tester(void)
  {
      ...
      for (NATIVE_INT_TYPE buffer = 0; buffer < m_numBuffers; buffer++) {
          // initialize buffers
          alloc.deallocate(0,this->m_readData[buffer]);
      }
